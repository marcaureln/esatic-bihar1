...
int main(int argc, char *argv[])
{
    if (argc < 5)
    {
        ...
        printf("./evenements <hote> <utilisateur> <mdp> <nom> [ignorer-initialisation]\n");
    ...
    // Si l'utilisateur saisi un 5ème argument, on ignore l'initialisation de la base de données.
    if (argc == 6)
    {
        printf("L'initialisation de la base de données a été ignorée !\n");
    }
    else
    {
    ...
    while (1)
    {
        // Affichage de la liste des évènements et des actions disponibles.
        ...
        // Le choix doit être compris entre 1 et l'id du dernier évènement. Si ce n'est pas le cas, on affiche un message d'erreur.
        if (choix >= 0 && choix <= idDernierEvenement)
        {
        ...
        {
            printf("\nVeuillez choisir une option dans la liste !\n");
            continue;
        }
    }
